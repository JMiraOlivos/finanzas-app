...
export async function loadForecastFile(params: {
  buffer: Buffer;
  filename: string;
  uploadedBy?: string;
  ...
  for (const pl of pnlLines) lineMap.set(pl.code.toLowerCase(), pl.id);

  const unknownLines = lineCodes.filter((c) => !lineMap.has(c.toLowerCase()));
  if (unknownLines.length > 0) {
    return { success: false, error: `Líneas PnL no encontradas: ${unknownLines.join(", ")}` };
  }
  ...
  try {
    const txResult = await sql.begin(async (sql) => {
      ...
      for (const [companyId, years] of companyYears) {
        for (const year of years) {
          await sql`
            UPDATE finanzas.forecast_versions
            SET is_active = FALSE
            WHERE company_id = ${companyId}::uuid AND year = ${year} AND is_active = TRUE
          `;
          const versionName = `${filename} — ${new Date().toISOString().slice(0, 10)}`;
          const [vRow] = await sql<{ id: string }[]>`
            INSERT INTO finanzas.forecast_versions (company_id, name, year, created_by)
      ...
      const versionMap = new Map(createdVersions.map((v) => [`${v.companyId}:${v.year}`, v.id]));
      ...
      for (let i = 0; i < values.length; i += BATCH) {
        ...
        await sql`
          INSERT INTO finanzas.forecast_monthly
    ...
    return { success: false, error: `Error al guardar forecast: ${(err as Error).message}` };
  }

  await logAudit({
    userId: params.uploadedBy ?? null,
    action: "upload_forecast",
    entityType: "forecast_version",
    metadata: { filename, row_count: parsed.rows.length, version_ids: versionIds, companies: companyNames },
  });

  return { success: true, versionIds, rowCount: parsed.rows.length, companiesLoaded: companyNames, warnings };
}
