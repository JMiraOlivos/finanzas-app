...
-- actual reads from fct_pnl_monthly (dbt mart); ly reads from v_pnl_base_monthly
-- (live view on journal_entries) so LY appears immediately after upload.
...
BEGIN
  ...
  WITH
    ...
    -- ── LY YTD (live view — no dbt refresh required) ─────────────────────────────
