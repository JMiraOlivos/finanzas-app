-- Migration 032: update fn_pnl_ytd / fn_pnl_lmonth_ytd / fn_pnl_monthly
-- to read the line list from pnl_lines_versioned (active published version)
-- instead of the legacy pnl_lines table.
-- This ensures lines added to a published structure version appear in EERR.
  ...
  WITH
    ...
    subtotals_lm AS (
      ...
      LEFT JOIN detail_lm d ON d.company_id = a.id AND d.line_code IN (
        SELECT code FROM active_lines WHERE parent_code = pl.code
      )
      GROUP BY a.id, pl.code
    ),
    base_lm AS (
